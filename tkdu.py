...
import math, tkinter, tkinter.filedialog, sys, os, stat, string, time, gzip
...
def string_count(bigger_string, search_for):
    return bigger_string.count(search_for)
...
def allocate(path, files, canvas, x, y, w, h, first, depth):
    ...
    if path and path[-1] == "/":
        ...
        nslashes = string_count(path, os.sep) - 1
    else:
        basename_idx = len(path) + 1
        nslashes = string_count(path, os.sep)
...
def reconfigure(e):
    ...
    if c.cur == "/":
        ...
        nslashes = string_count(c.cur, os.sep) - 1
    parent = os.path.dirname(c.cur)
...
def doit(dir, files):
    ...
    for k, v in files.items():
        sv = list(map(lambda item: [item[1], item[0], None], v.items()))
        sv.sort()
        sv.reverse()
        sorted_files[k] = (v, sv)
...
def du(dir, files, fs=0, ST_MODE=stat.ST_MODE, ST_SIZE=stat.ST_SIZE, S_IFMT=0o170000, S_IFDIR=0o040000, lstat=os.lstat,
